      ...
      <aside className="w-16 md:w-64 border-r border-border bg-card flex flex-col justify-between">
        <div>
          ...
          <nav className="p-2 md:p-4 space-y-2">
            <NavItem href="/chat" icon={<MessageSquare className="w-5 h-5" />} label="Comms" active={location === "/chat"} />
            <NavItem href="/settings" icon={<Settings className="w-5 h-5" />} label="Settings" active={location === "/settings"} />
            <NavItem href="/admin" icon={<Users className="w-5 h-5" />} label="Network" active={location === "/admin"} />
          </nav>
        </div>
        <div className="p-2 md:p-4 border-t border-border">
          ...
          >
            ...
            <span className="hidden md:inline ml-3 font-mono text-sm uppercase">Disconnect</span>
          </button>
        </div>
      </aside>
...
function NavItem({ href, icon, label, active }: { href: string; icon: React.ReactNode; label: string; active: boolean }) {
  return (
    <Link href={href} className={`flex items-center justify-center md:justify-start px-3 py-3 transition-colors uppercase text-sm font-mono ${active ? "bg-primary/20 text-primary border-l-2 border-primary" : "text-muted-foreground hover:bg-accent/50 hover:text-foreground border-l-2 border-transparent"}`}>
      {icon}
