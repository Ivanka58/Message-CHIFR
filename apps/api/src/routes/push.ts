...
import { db, pushSubscriptionsTable } from "@shifr/db";
...
export async function sendPushToUser(toUserId: number, notification: { title: string; body: string; tag?: string }) {
  if (!VAPID_PUBLIC || !VAPID_PRIVATE) return;
  const subs = await db.select().from(pushSubscriptionsTable).where(eq(pushSubscriptionsTable.userId, toUserId));
...
router.post("/push/subscribe", requireSession, async (req: AuthedRequest, res) => {
  ...
  if (!endpoint || !keys?.p256dh || !keys?.auth) { res.status(400).json({ error: "Invalid subscription payload" }); return; }
  await db.insert(pushSubscriptionsTable).values({ userId, endpoint, p256dh: keys.p256dh, auth: keys.auth })
