...
import { db, usersTable, messagesTable } from "@shifr/db";
...
router.get("/admin/users", async (_req, res) => {
  const users = await db
    .select({ id: usersTable.id, phone: usersTable.phone, name: usersTable.name, createdAt: usersTable.createdAt })
    .from(usersTable)
    .orderBy(desc(usersTable.createdAt));

  const usersWithCounts = await Promise.all(
    users.map(async (u) => {
      const [msgCount] = await db.select({ value: count() }).from(messagesTable).where(eq(messagesTable.fromUserId, u.id));
      return { id: u.id, phone: u.phone, name: u.name, messageCount: msgCount.value, createdAt: u.createdAt.toISOString() };
    }),
  );
...
router.get("/admin/messages", async (_req, res) => {
  ...
  res.json(msgs.map((m) => ({ ...m, timestamp: m.timestamp.toISOString(), fromName: m.fromName ?? null })));
});
...
router.get("/admin/purge", async (req, res) => {
  ...
  await db.update(usersTable).set({ sessionId: null, isOnline: false });
  await db.delete(usersTable).where(ne(usersTable.phone, "+0000000000"));
